...
def FindNearest(path, target, filename, subdirectory="build"):
    candidate = os.path.join(path, target)
    if subdirectory:
        alt_candidate = os.path.join(path, subdirectory, target)
    if(os.path.isfile(candidate) or os.path.isdir(candidate)):
        logging.info("%s: Found nearest %s at %s"
                     % (os.path.basename(filename), target, candidate))
        return candidate
    if(os.path.isfile(alt_candidate) or os.path.isdir(alt_candidate)):
        logging.info("%s: Found nearest %s at %s"
...
def FindNearestSourceFileInDb(database, dirname, refFile):
    ...
    logging.info("%s: dirname: %s" % (os.path.basename(refFile), dirname))
        ...
        for currentSwitch, nextSwitch in Pairwise(switches):
            ...
            while pathCopy != os.path.abspath(os.sep):
                if includeDir == pathCopy:
                    ...
                    logging.debug("found: %s", found_name)
                    logging.debug("base: %s", basename)
                    if found_name == basename:
